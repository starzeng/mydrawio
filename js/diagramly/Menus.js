...
(function () {
  ...
  Menus.prototype.init = function () {
    ...
    var googleEnabled = ((urlParams['embed'] != '1' && urlParams['gapi'] != '0')
        ...
        && mxClient.IS_SVG &&
        isLocalStorage && (document.documentMode == null
            || document.documentMode >= 10);
    var dropboxEnabled = ((urlParams['embed'] != '1' && urlParams['db'] != '0')
    ...
    editorUi.actions.addAction('new...', function () {
      ...
      if (!compact && urlParams['newTempDlg'] == '1' && editorUi.mode
          == App.MODE_GOOGLE) {
        ...
        var tempDlg = new TemplatesDialog(editorUi,
            function (templateXml, title, infoObj) {
              ...
              editorUi.pickFolder(editorUi.mode, function (folderId) {
                editorUi.createFile(title, templateXml, (templateLibs != null &&
                        templateLibs.length > 0) ? templateLibs : null, null,
                    function () {
                      editorUi.hideDialog();
              ...
              editorUi.drive.listFiles(null, oneWeek, username ? true : false,
                  function (resp) {
                    var results = [];
              ...
              editorUi.drive.listFiles(str, null, username ? true : false,
                  function (resp) {
                    var results = [];
        ...
        editorUi.showDialog(tempDlg.container, window.innerWidth,
      ...
      var dlg = new NewDialog(editorUi, compact,
          !(editorUi.mode == App.MODE_DEVICE && 'chooseFileSystemEntries'
              in window));

      editorUi.showDialog(dlg.container, (compact) ? 350 : 620,
          (compact) ? 70 : 460, true, true, function (cancel) {
    ...
    editorUi.actions.put('insertTemplate',
    ...
    var shareCursorAction = editorUi.actions.addAction('shareCursor',
        function () {
    ...
    this.put('units', new Menu(mxUtils.bind(this, function (menu, parent) {
      this.addMenuItems(menu, ['points', 'inches', 'millimeters', 'meters'],
          parent);
    ...
    var fullscreenAction = editorUi.actions.addAction('fullscreen',
        function () {
    ...
    var lightModeAction = editorUi.actions.put('lightMode',
    ...
    var darkModeAction = editorUi.actions.put('darkMode',
    ...
    var autoModeAction = editorUi.actions.put('autoMode',
    ...
    var toggleSimpleModeAction = editorUi.actions.put('toggleSimpleMode',
    ...
    toggleSimpleModeAction.visible = Editor.currentTheme != 'min'
        && Editor.currentTheme != 'sketch' &&
    ...
    var toggleSketchModeAction = editorUi.actions.put('toggleSketchMode',
    ...
    if (window.mxFreehand) {
      editorUi.actions.put('insertFreehand',
          new Action(mxResources.get('freehand') + '...', function () {
            if (graph.isEnabled()) {
    ...
    editorUi.actions.put('exportXml',
        new Action(mxResources.get('formatXml') + '...', function () {
          ...
          var compressed = editorUi.addCheckbox(div,
              mxResources.get('compressed'), Editor.defaultCompressed);
    ...
    if (Editor.enableExportUrl) {
      editorUi.actions.put('exportUrl',
          new Action(mxResources.get('url') + '...', function () {
            editorUi.showPublishLinkDialog(mxResources.get('url'), true, null,
                null,
                function (linkTarget, linkColor, allPages, lightbox, editLink,
                    layers, width, height, tags) {
                  var params = [];
                  ...
                  var dlg = new EmbedDialog(editorUi,
                      editorUi.createLink(linkTarget, linkColor,
                          allPages, lightbox, editLink, layers, null, true,
                          params));
        ...
        new Action(mxResources.get('formatHtmlEmbedded') + '...', function () {
          if (editorUi.spinner.spin(document.body,
            ...
            editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
              ...
              editorUi.showHtmlDialog(mxResources.get('export'), null, url,
                  function (publicUrl, zoomEnabled,
                      initialZoom, linkTarget, linkColor, fit, allPages, layers,
                      tags, lightbox, editLink) {
                    editorUi.createHtml(publicUrl, zoomEnabled, initialZoom,
                        linkTarget, linkColor, fit, allPages,
                        layers, tags, lightbox, editLink,
                        mxUtils.bind(this, function (html, scriptTag) {
                          ...
                          var result = '<!--[if IE]><meta http-equiv="X-UA-Compatible" content="IE=5,IE=9" ><![endif]-->\n'
                              +
                              '<!DOCTYPE html>\n<html>\n<head>\n<title>'
                              + mxUtils.htmlEntities(basename) + '</title>\n' +
                              '<meta charset="utf-8"/>\n</head>\n<body>' + html
                              + '\n' + scriptTag + '\n</body>\n</html>';
                          editorUi.saveData(basename + ((basename.substring(
                                  basename.lenth - 7) ==
                              '.drawio') ? '' : '.drawio') + '.html', 'html',
                              result, 'text/html');
                        }));
                  });
            });
    ...
    editorUi.actions.put('exportPdf',
        new Action(mxResources.get('formatPdf') + '...', function () {
          if (!EditorUi.isElectronApp && (editorUi.isOffline()
              || editorUi.printPdfExport)) {
            ...
            if (editorUi.pdfPageExport && !noPages) {
              ...
              var pagesRadio = editorUi.addRadiobox(div, 'pages',
                  mxResources.get('pages') + ':', false, null, true);
              ...
              var crop = editorUi.addCheckbox(div, mxResources.get('crop'),
                  false, true);
              var grid = editorUi.addCheckbox(div, mxResources.get('grid'),
                  false, false);
            ...
            var dlg = new CustomDialog(editorUi, div,
                mxUtils.bind(this, function () {
                  ...
                  editorUi.downloadFile('pdf', null, null, !selection.checked,
                      ...
                      !crop.checked,
                      transparentBkg != null && transparentBkg.checked, null,
    ...
    editorUi.actions.addAction('editShape...', mxUtils.bind(this, function () {
      ...
      if (graph.getSelectionCount() == 1) {
        ...
        if (state != null && state.shape != null && state.shape.stencil
            != null) {
    ...
    editorUi.actions.addAction('upload...', function () {
      ...
      if (file != null) {
        ...
        var filename = (file.getTitle() != null) ? file.getTitle()
            : editorUi.defaultFilename;
        editorUi.openLink(window.location.protocol + '//' + window.location.host
            + '/?create=drawdata&' +
    ...
    editorUi.actions.addAction('pasteStyle', function () {
      if (graph.isEnabled() && !graph.isSelectionEmpty() && editorUi.copiedStyle
          != null) {
          ...
          editorUi.showExportDialog(mxResources.get('formatSvg'), true,
              ...
              mxUtils.bind(this,
                  function (scale, transparentBackground, ignoreSelection,
                      addShadow, editable, embedImages, border, cropImage,
                      currentPage,
                    ...
                    if (!isNaN(val) && val > 0) {
                      if (lblToSvg) {
                        editorUi.downloadFile('remoteSvg', null, null,
                            ignoreSelection, null, cropImage,
                            transparentBackground, scale, border, null,
                            editable);
                      } else {
                        editorUi.exportSvg(val / 100, transparentBackground,
                            ignoreSelection,
      ...
      if (editorUi.isExportToCanvas()) {
        editorUi.showExportDialog(mxResources.get('image'), false,
            ...
            mxUtils.bind(this,
                function (scale, transparentBackground, ignoreSelection,
                    addShadow, editable,
                    embedImages, border, cropImage, currentPage, dummy, grid,
                    theme, exportType) {
                  var val = parseInt(scale);
                  ...
                  if (!isNaN(val) && val > 0) {
                    editorUi.exportImage(val / 100,
                        ...
                        ignoreSelection, addShadow, editable && format == 'png',
                        border,
                        !cropImage, false, format, grid, null, theme,
                        exportType);

                    if (done != null) {
                      done(scale, transparentBackground, ignoreSelection,
                          addShadow, editable, embedImages,
                          border, cropImage, currentPage, dummy, grid, theme,
                          exportType);
                    }
                  }
                }), true, defaultEditable, format, true);
      } else if (!editorUi.isOffline() && (!mxClient.IS_IOS
          || !navigator.standalone)) {
        editorUi.showRemoteExportDialog(mxResources.get('export'), null,
            mxUtils.bind(this,
                function (ignoreSelection, editable, transparent, scale,
                    border) {
                  editorUi.downloadFile(
                      (editable && format == 'png') ? 'xmlpng' : format,
                      null, null, ignoreSelection, null, null, transparent,
                      scale, border);
                }), false, true);
      }
    };
    ...
    editorUi.actions.addAction('keyboardShortcuts...', function () {
      if (mxClient.IS_SVG && !mxClient.IS_CHROMEAPP
    ...
    action = editorUi.actions.addAction('tags', mxUtils.bind(this, function () {
      if (this.tagsWindow == null) {
        this.tagsWindow = new TagsWindow(editorUi,
            document.body.offsetWidth - 400, 60, 212, 200);
        this.tagsWindow.window.addListener('show',
            mxUtils.bind(this, function () {
    ...
    action = editorUi.actions.addAction('findReplace',
        mxUtils.bind(this, function (arg1, evt) {
          var findReplace = graph.isEnabled() && (evt == null
    ...
    if (isLocalStorage && localStorage != null && urlParams['embed'] != '1') {
      editorUi.actions.addAction('configuration...', function () {
        ...
        if (!editorUi.isOfflineApp() && isLocalStorage && editorUi.mode
            != App.MODE_ATLAS) {
          var pluginsAction = editorUi.actions.get('plugins');

          if (pluginsAction != null && (Editor.currentTheme == 'sketch' ||
              Editor.currentTheme == 'simple' || Editor.currentTheme
              == 'min')) {
        ...
        if (editorUi.getServiceName() != 'atlassian' && urlParams['embed']
            != '1') {
          buttons.push([mxResources.get('preferences'), function () {
            editorUi.showLocalStorageDialog(
                mxResources.get('preferences') + ':', Editor.settingsKey,
        ...
        editorUi.showLocalStorageDialog(mxResources.get('configuration') + ':',
    ...
    if (mxClient.IS_CHROMEAPP || isLocalStorage) {
      this.put('language', new Menu(mxUtils.bind(this, function (menu, parent) {
        var addLangItem = mxUtils.bind(this, function (id) {
          var lang = (id == '') ? mxResources.get('automatic')
              : mxLanguageMap[id];
          var item = null;
      ...
      Menus.prototype.createMenubar = function (container) {
        ...
        if ((urlParams['embed'] != '1' || urlParams['atlas'] == '1') &&
          ...
          if (themeMenu != null) {
            ...
            this.editorUi.addListener('currentThemeChanged',
                updateThemeElement);
    ...
    editorUi.actions.put('convertDarkModeColors',
    ...
    layoutMenu.funct = function (menu, parent) {
      ...
      menu.addItem(mxResources.get('orgChart'), null, function () {
        var branchOptimizer = null, parentChildSpacingVal = 20,
            siblingSpacingVal = 20;

        // Invoked when orgchart code was loaded
        var delayed = function () {
          if (typeof mxOrgChartLayout !== 'undefined' && branchOptimizer
              != null) {
        ...
        var customBtn = mxUtils.button(mxResources.get('custom') + '...',
            function () {
              var value = [{
      ...
      menu.addItem(mxResources.get('parallels'), null,
          mxUtils.bind(this, function () {
            editorUi.tryAndHandle(mxUtils.bind(this, function () {
              var layout = new mxParallelEdgeLayout(graph);
              layout.checkOverlap = true;

              editorUi.prompt(mxResources.get('spacing'), layout.spacing,
      ...
      menu.addSeparator(parent);
      editorUi.menus.addMenuItem(menu, 'runLayout', parent, null, null,
    ...
    if (urlParams['test'] == '1') {
      ...
      editorUi.actions.addAction('createSidebarEntry',
          mxUtils.bind(this, function () {
      ...
      editorUi.actions.addAction('showBoundingBox',
          mxUtils.bind(this, function () {
            ...
            function checkBounds(shape) {
              return shape == null || shape.boundingBox == null || (!isNaN(
                      shape.boundingBox.x) &&
      ...
      editorUi.actions.addAction('testCheckFile',
          mxUtils.bind(this, function () {
            var xml = (editorUi.pages != null && editorUi.getCurrentFile()
                != null) ?
                editorUi.getCurrentFile().getAnonymizedXmlForPages(
                    editorUi.pages) : '';
                ...
                function (newValue) {
                  if (newValue.length > 0) {
                    try {
                      ...
                      var pages = editorUi.getPagesForNode(doc.documentElement,
                          'mxGraphModel');
                      ...
                      function checkModel(node) {
                        ...
                        if (root == null) {
                          ...
                          if (Object.keys(visited).length != Object.keys(
                              allIds).length) {
                            mxLog.debug(
                                pageId + ': Invalid tree: (see console)');
      ...
      editorUi.actions.addAction('testDiff', mxUtils.bind(this, function () {
        if (editorUi.pages != null) {
          var buttons = [['Snapshot', function (evt, input) {
            ...
            dlg.textarea.value = 'Snapshot updated '
          ...
          var dlg = new TextareaDialog(editorUi, 'Diff/Sync:', '',
              ...
              }, null, 'Close', null, null, null, true, null, 'Patch', null,
              buttons);

          if (snapshot == null) {
            snapshot = editorUi.getPagesForXml(editorUi.getFileData(true));
            dlg.textarea.value = 'Snapshot created '
                + new Date().toLocaleString() +
                ' Checksum ' + editorUi.getHashValueForPages(snapshot);
          } else {
            dlg.textarea.value = JSON.stringify(editorUi.diffPages(
      ...
      editorUi.actions.addAction('testChecksum',
          mxUtils.bind(this, function () {
            var file = editorUi.getCurrentFile();
      ...
      editorUi.actions.addAction('testCheckPages',
          mxUtils.bind(this, function () {
            var file = editorUi.getCurrentFile();
      ...
      editorUi.actions.addAction('testFixPages',
          mxUtils.bind(this, function () {
      ...
      editorUi.actions.addAction('testOptimize',
          mxUtils.bind(this, function () {
            ...
            try {
              ...
              console.log('Removed', imageCount, 'image(s) from', cells.length,
                  'cell(s): ', [cells, images]);
            } finally {
      ...
      editorUi.actions.addAction('testXmlImageExport',
          mxUtils.bind(this, function () {
            var scale = 1;
            var b = 1;
      ...
      editorUi.actions.addAction('testPerformance',
          mxUtils.bind(this, function () {
    ...
    editorUi.actions.put('shapes',
        new Action(mxResources.get('moreShapes') + '...', function (evt) {
          if (mxClient.IS_CHROMEAPP || !editorUi.isOffline()) {
            editorUi.showDialog(new MoreShapesDialog(editorUi, true).container,
                640, (isLocalStorage) ?
                    ((mxClient.IS_IOS) ? 480 : 460) : 440, true, true);
          } else {
            editorUi.showDialog(new MoreShapesDialog(editorUi, false).container,
                360, (isLocalStorage) ?
                    ((mxClient.IS_IOS) ? 300 : 280) : 260, true, true);
          }
        }));

    editorUi.actions.put('createShape',
          ...
          if (editorUi.spinner.spin(document.body,
            ...
            editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
              ...
              editorUi.showHtmlDialog(mxResources.get('create'),
                  ...
                  url,
                  function (publicUrl, zoomEnabled, initialZoom, linkTarget,
                      linkColor, fit, allPages, layers, tags, lightbox,
                      editLink) {
                    editorUi.createHtml(publicUrl, zoomEnabled, initialZoom,
                        linkTarget, linkColor, fit, allPages,
                        layers, tags, lightbox, editLink,
                              ...
                              html + '\n' + scriptTag, null, null, function () {
                                ...
                                if (doc != null) {
                                  ...
                                  if (!direct) {
                                    ...
                                    wnd.document.body.insertBefore(info,
    ...
    editorUi.actions.put('liveImage', new Action('Live image...', function () {
      ...
      if (current != null && editorUi.spinner.spin(document.body,
          mxResources.get('loading'))) {
        editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
          editorUi.spinner.stop();
          ...
          if (url != null) {
            var dlg = new EmbedDialog(editorUi,
                '<img src="' + ((current.constructor != DriveFile) ?
        ...
        new Action(mxResources.get('image') + '...', function () {
          editorUi.showEmbedImageDialog(
              function (fit, shadow, retina, lightbox, editLink, layers) {
                if (editorUi.spinner.spin(document.body,
                    mxResources.get('loading'))) {
                  editorUi.createEmbedImage(fit, shadow, retina, lightbox,
                      editLink, layers, function (result) {
                        editorUi.spinner.stop();
        ...
        new Action(mxResources.get('formatSvg') + '...', function () {
          editorUi.showEmbedImageDialog(
              function (fit, shadow, image, lightbox, editLink, layers) {
                if (editorUi.spinner.spin(document.body,
                    mxResources.get('loading'))) {
                  editorUi.createEmbedSvg(fit, shadow, image, lightbox,
                      editLink, layers, function (result) {
                        editorUi.spinner.stop();
                ...
                if (editorUi.spinner.spin(document.body,
                  ...
                  editorUi.getPublicUrl(editorUi.getCurrentFile(),
                      function (url) {
                        ...
                        var dlg = new EmbedDialog(editorUi,
                            '<iframe frameborder="0" style="width:' + width +
                            ';height:' + height + ';" src="'
                            + editorUi.createLink(linkTarget, linkColor,
                                allPages, lightbox, editLink, layers, url, null,
                                params) + '"></iframe>');
                        editorUi.showDialog(dlg.container, 450, 240, true,
                            true);
                        dlg.init();
    ...
    editorUi.actions.put('embedNotion',
        new Action(mxResources.get('notion') + '...', function () {
          ...
          editorUi.showPublishLinkDialog(mxResources.get('notion'), null, null,
              null,
              function (linkTarget, linkColor, allPages, lightbox, editLink,
                  layers, width, height, tags) {
                if (editorUi.spinner.spin(document.body,
    ...
    if (isLocalStorage || mxClient.IS_CHROMEAPP) {
      ...
      if (urlParams['plugins'] != '0') {
        editorUi.actions.addAction('plugins...', function () {
          editorUi.showDialog(new PluginsDialog(editorUi).container, 380, 240,
              true, false);
        });
      }
    }
    ...
    if (urlParams['embed'] == '1') {
      editorUi.actions.get('save').funct = function (exit) {
        ...
        var data = (urlParams['pages'] != '0' || (editorUi.pages != null
            ...
            editorUi.getFileData(true) : mxUtils.getXml(
        ...
        if (file != null && file.constructor != EmbedFile && (file.constructor
            != LocalFile || file.mode != null)) {
          editorUi.saveFile();
        }
      };

      var saveAndExitAction = editorUi.actions.addAction('saveAndExit',
          function () {
    ...
    this.put('exportAs', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      this.addMenuItems(menu, ['-', 'exportHtml', 'exportXml', 'exportUrl'],
          parent);
    ...
    this.put('importFrom', new Menu(mxUtils.bind(this, function (menu, parent) {
      var doImportFile = mxUtils.bind(this, function (data, mime, filename) {
        ...
        var x = graph.snap(Math.ceil(
            Math.max(0, bds.x / view.scale - view.translate.x) + 4
            * graph.gridSize));
        var y = graph.snap(Math.ceil(
            ...
            var doInsert = mxUtils.bind(this, function () {
              editorUi.resizeImage(img, data,
                  mxUtils.bind(this, function (data2, w2, h2) {
                    var s = (resizeImages) ? Math.min(1,
                        Math.min(editorUi.maxImageSize / w2,
                            editorUi.maxImageSize / h2)) : 1;

                    editorUi.importFile(data, mime, x, y, Math.round(w2 * s),
                        Math.round(h2 * s), filename, function (cells) {
                          editorUi.spinner.stop();
        ...
        service.pickFile(function (id) {
          if (editorUi.spinner.spin(document.body,
            ...
            service.getFile(id, function (file) {
                  var mime = (file.getData().substring(0, 11) == 'data:image/')
                      ? getMimeType(file.getTitle()) : 'text/xml';

                  // Imports SVG as images
                  if (/\.svg$/i.test(file.getTitle()) && !editorUi.editor.isDataSvg(
                      file.getData())) {
                ...
                function (resp) {
                  editorUi.handleError(resp,
                      (resp != null) ? mxResources.get('errorLoadingFile')
                          : null);
      ...
      if (typeof (google) != 'undefined' && typeof (google.picker)
          != 'undefined') {
      ...
      } else if (oneDriveEnabled && typeof window.OneDriveClient
          === 'function') {
        menu.addItem(
      ...
      if (!editorUi.isOffline()) {
        ...
        menu.addItem(mxResources.get('url') + '...', null, function () {
          var dlg = new FilenameDialog(editorUi, '', mxResources.get('import'),
              function (fileUrl) {
                if (fileUrl != null && fileUrl.length > 0
                    && editorUi.spinner.spin(document.body,
                        mxResources.get('loading'))) {
                  var mime = (/(\.png)($|\?)/i.test(fileUrl)) ? 'image/png'
                      : 'text/xml';
    ...
    this.put('dynamicAppearance',
        new Menu(mxUtils.bind(this, function (menu, parent) {
          ...
          if (Editor.currentTheme == 'simple') {
            ...
            if (iw < 750) {
              this.addSubmenu('viewZoom', menu, parent,
    ...
    this.put('appearance', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (!editorUi.isOffline() || mxClient.IS_CHROMEAPP
          || EditorUi.isElectronApp) {
        editorUi.menus.addLinkToItem(item,
            'https://github.com/jgraph/drawio/discussions/3701');
      }
    })));

    editorUi.actions.addAction('addToScratchpad', function (evt) {
      if (!graph.isSelectionEmpty() && editorUi.addSelectionToScratchpad
          != null) {
          ...
          if (file != null) {
            if (file.constructor == LocalFile && file.fileHandle != null) {
              ...
              var dlg = new FilenameDialog(this.editorUi, filename,
                  ...
                  mxUtils.bind(this, function (title) {
                  ...
                  }), (file.constructor == DriveFile || file.constructor
                      == StorageFile) ?
                    ...
                    editorUi.showError(mxResources.get('error'),
                        mxResources.get('invalidName'), mxResources.get('ok'));
                ...
                if (mode == '_blank') {
                  ...
                  if (newTitle != null && newTitle.length > 0) {
                    if (mode == App.MODE_GOOGLE) {
                      ...
                      editorUi.createFile(newTitle, editorUi.getFileData(true),
                          null, mode);
                    }
                  }
                }
    ...
    editorUi.actions.put('openFolder', new Action(
    ...
    editorUi.actions.addAction('moveToFolder...',
        mxUtils.bind(this, function () {
          ...
          if (file.getMode() == App.MODE_GOOGLE || file.getMode()
              == App.MODE_ONEDRIVE) {
            var isInRoot = false;

            if (file.getMode() == App.MODE_GOOGLE && file.desc.parents
                != null) {
    ...
    this.put('embed', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (file != null && (file.getMode() == App.MODE_GOOGLE ||
          file.getMode() == App.MODE_GITHUB) && /(\.png)$/i.test(
          file.getTitle())) {
        this.addMenuItems(menu, ['liveImage', '-'], parent);
      }

      this.addMenuItems(menu, ['embedImage', 'embedSvg', '-', 'embedHtml'],
          parent);
      ...
      if (urlParams['embed'] != '1' && !editorUi.isOffline()) {
        this.addMenuItems(menu,
            ['-', 'googleDocs', 'googleSlides', 'googleSheets', '-',
              'microsoftOffice', '-', 'embedNotion'], parent);
      }
    })));
    ...
    var insertEdge = function (value, length, style, pt) {
      ...
      cell.geometry.setTerminalPoint(
          new mxPoint(pt.x + cell.geometry.width, pt.y), false);
    ...
    editorUi.actions.put('insertText',
        new Action(mxResources.get('text'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
            graph.startEditingAtCell(insertVertex('Text', 60, 30,
                'text;strokeColor=none;align=center;' +
                ...
                (evt != null && !mxEvent.isControlDown(evt)
                    ...
                    graph.isMouseInsertPoint()) ? graph.getInsertPoint()
                    : null));
          }
        }, null, null, 'A')).isEnabled = isGraphEnabled;

    editorUi.actions.put('insertRectangle',
        new Action(mxResources.get('rectangle'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
    ...
    editorUi.actions.put('insertNote',
        new Action(mxResources.get('note'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
            insertVertex('', 140, 160,
    ...
    editorUi.actions.put('insertEllipse',
        new Action(mxResources.get('ellipse'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
    ...
    editorUi.actions.put('insertRhombus',
        new Action(mxResources.get('rhombus'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
    ...
    editorUi.actions.put('insertEdge',
        new Action(mxResources.get('line'), function (evt) {
          if (graph.isEnabled() && !graph.isCellLocked(
              graph.getDefaultParent())) {
            insertEdge('', graph.defaultEdgeLength,
    ...
    var toggleShapes = editorUi.actions.put('toggleShapes',
        new Action(mxResources.get('shapes'), function () {
    ...
    toggleShapes.setSelectedCallback(mxUtils.bind(this, function () {
      return (editorUi.sidebarWindow != null
              && editorUi.sidebarWindow.window.isVisible()) ||
          (editorUi.sidebarWindow == null && editorUi.hsplitPosition > 0);
    }));

    editorUi.addInsertMenuItems = mxUtils.bind(this,
        function (menu, parent, methods) {
          for (var i = 0; i < methods.length; i++) {
            if (methods[i] == '-') {
              menu.addSeparator(parent);
            } else {
              editorUi.addInsertItem(menu, parent,
    ...
    this.put('insert', new Menu(mxUtils.bind(this, function (menu, parent) {
      if (Editor.currentTheme == 'sketch') {
        ...
        editorUi.menus.addMenuItems(menu, ['insertImage', 'insertLink', '-'],
            parent);
        editorUi.menus.addSubmenu('insertAdvanced', menu, parent,
            mxResources.get('advanced'));
        editorUi.menus.addSubmenu('layout', menu, parent);
      } else {
        this.addMenuItems(menu,
            ['insertRectangle', 'insertEllipse', 'insertRhombus',
        ...
        this.addSubmenu('insertAdvanced', menu, parent,
            mxResources.get('advanced'));
      }
    })));
      ...
      if (recent != null) {
        for (var i = 0; i < recent.length; i++) {
          (function (entry) {
            ...
            menu.addItem(entry.title + ' (' + mxResources.get(modeKey) + ')',
                null, function () {
    ...
    this.put('openFrom', new Menu(function (menu, parent) {
      ...
      if (!editorUi.isOffline()) {
        ...
        menu.addItem(mxResources.get('url') + '...', null, function () {
          var dlg = new FilenameDialog(editorUi, '', mxResources.get('open'),
              function (fileUrl) {
                if (fileUrl != null && fileUrl.length > 0) {
    ...
    if (Editor.enableCustomLibraries) {
      this.put('newLibrary', new Menu(function (menu, parent) {
        if (typeof (google) != 'undefined' && typeof (google.picker)
          ...
          } else if (googleEnabled && typeof window.DriveClient
              === 'function') {
            menu.addItem(
                mxResources.get('googleDrive') + ' (' + mxResources.get(
                    'loading') + '...)', null, function () {
                  // do nothing
                }, parent, null, false);
          }
        }
        ...
        } else if (dropboxEnabled && typeof window.DropboxClient
            === 'function') {
          menu.addItem(
              mxResources.get('dropbox') + ' (' + mxResources.get('loading')
              + '...)', null, function () {
      ...
      this.put('openLibraryFrom', new Menu(function (menu, parent) {
                ...
                function (fileUrl) {
                  if (fileUrl != null && fileUrl.length > 0
                    ...
                    if (!editorUi.editor.isCorsEnabledForUrl(fileUrl)) {
                      realUrl = PROXY_URL + '?url=' + encodeURIComponent(
                          fileUrl);
                    }
        ...
        if (urlParams['confLib'] == '1') {
          ...
          menu.addItem(mxResources.get('confluenceCloud') + '...', null,
              function () {
    ...
    this.put('edit', new Menu(mxUtils.bind(this, function (menu, parent) {
      this.addMenuItems(menu,
          ['undo', 'redo', '-', 'cut', 'copy', 'copyAsImage', 'paste',
            'delete', '-', 'duplicate', '-', 'findReplace', '-', 'editData',
            'editTooltip', '-',
            'editStyle', 'editGeometry', '-', 'edit', '-', 'editLink',
            'openLink', '-',
            'selectVertices', 'selectEdges', 'selectAll', 'selectNone', '-',
            'lockUnlock']);
    })));

    var action = editorUi.actions.addAction('comments',
        mxUtils.bind(this, function () {
          if (this.commentsWindow == null) {
            // LATER: Check outline window for initial placement
            this.commentsWindow = new CommentsWindow(editorUi,
                document.body.offsetWidth - 380, 120, 300, 350);
    ...
    action.setToggleAction(true);
    action.setSelectedCallback(mxUtils.bind(this, function () {
      return this.commentsWindow != null
          && this.commentsWindow.window.isVisible();
    }));
    ...
    this.put('editCell', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      this.addMenuItems(menu,
          ['editLink', 'editShape', 'editImage', 'crop', '-',
            'editData', 'copyData', 'pasteData', '-', 'editConnectionPoints',
            'editGeometry', '-', 'editTooltip', 'editStyle', '-', 'edit'],
          parent);
    })));

    // Current page menu
    this.put('currentPage',
        new Menu(mxUtils.bind(this, function (menu, parent) {
          var page = editorUi.currentPage;

          if (page != null) {
            menu.addItem(mxResources.get('rename') + '...', null,
                mxUtils.bind(this, function () {
                  editorUi.renamePage(page, page.getName());
                }), parent);

            menu.addItem(mxResources.get('delete'), null,
                mxUtils.bind(this, function () {
                  editorUi.removePage(page);
                }), parent);

            if (editorUi.pages.length > 1) {
              editorUi.menus.addSubmenu('movePage', menu, parent,
            ...
            menu.addItem(mxResources.get('duplicate'), null,
                mxUtils.bind(this, function () {
                  editorUi.duplicatePage(page,
            ...
            if (urlParams['embed'] != 1) {
              ...
              if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp
                  && editorUi.getServiceName() == 'draw.io') {
                menu.addItem(mxResources.get('openInNewWindow'), null,
                    mxUtils.bind(this, function () {
                      editorUi.editor.editAsNew(
                          editorUi.getFileData(true, null, null, null, true,
                              true));
                    }), parent);
              }
              ...
              if (url != null) {
                menu.addItem(mxResources.get('link') + '...', null,
                    mxUtils.bind(this, function () {
    ...
    this.put('pages', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (!editorUi.editor.graph.isLightboxView()) {
        menu.addItem(mxResources.get('insertPage'), null,
            mxUtils.bind(this, function () {
              editorUi.insertPage();
            }), parent);
    ...
    if (EditorUi.isElectronApp) {
      ...
      var spellCheckAction = editorUi.actions.addAction('spellCheck',
          function () {
      ...
      var googleFontsAction = editorUi.actions.addAction('googleFonts',
          function () {
      ...
      editorUi.actions.addAction('drafts...', function () {
        var dlg = new FilenameDialog(editorUi,
            (EditorUi.draftSaveDelay / 1000) + '', mxResources.get('apply'),
            mxUtils.bind(this, function (newValue) {
            ...
            }), mxResources.get('draftSaveInt'), null, null, null, null, null,
            null, 50, 250);
    ...
    this.put('extras', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      var sketchTheme = Editor.currentTheme == 'simple' || urlParams['sketch']
          == '1';
      ...
      if (sketchTheme || uiTheme == 'min') {
        ...
        if (langMenu != null && (urlParams['embed'] != '1' || urlParams['lang']
            == null)) {
        ...
        if (EditorUi.isElectronApp) {
          this.addMenuItems(menu,
              ['googleFonts', 'spellCheck', 'autoBkp', 'drafts', '-'], parent);
        }
          ...
          editorUi.actions.addAction('templates', function () {
            ...
            var tempDlg = new TemplatesDialog(editorUi, function (xml) {
                ...
                null, null, 'user', function (callback, error, username) {
                  ...
                  editorUi.drive.listFiles(null, oneWeek,
                      username ? true : false, function (resp) {
                        var results = [];
    ...
    this.put('movePage', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (editorUi.pages != null) {
        for (var i = 0; i < editorUi.pages.length; i++) {
          if (i != current) {
            (function (index) {
              menu.addItem(editorUi.getShortPageName(editorUi.pages[index]),
                  null, function () {
    ...
    this.put('diagram', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (Editor.currentTheme != 'simple') {
        editorUi.menus.addSubmenu('extras', menu, parent,
      ...
      if (mxClient.IS_CHROMEAPP || EditorUi.isElectronApp) {
        ...
        if (urlParams['saveAndExit'] == '1' ||
            (urlParams['noSaveBtn'] == '1' &&
                urlParams['saveAndExit'] != '0') || editorUi.mode
            == App.MODE_ATLAS) {
      ...
      if (Editor.currentTheme != 'simple' && Editor.currentTheme != 'min') {
        ...
        editorUi.menus.addMenuItems(menu,
            ['toggleShapes', 'format', 'layers', 'tags', '-'], parent);
      ...
      if (urlParams['noFileMenu'] != '1' && (!mxClient.IS_IOS
          || !navigator.standalone)) {
    ...
    this.put('save', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (file != null && (file.constructor == DriveFile || file.constructor
          == OneDriveFile)) {
        editorUi.menus.addMenuItems(menu,
            ['save', 'makeCopy', '-', 'rename', 'moveToFolder'], parent);
      } else {
        editorUi.menus.addMenuItems(menu, ['save', 'saveAs', '-', 'rename'],
            parent);
    ...
    this.put('file', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (urlParams['embed'] == '1') {
        ...
        this.addMenuItems(menu, ['-', 'pageSetup', 'print', '-', 'rename'],
            parent);
    ...
    ChangeExtFonts.prototype.execute = function () {
      ...
      this.ui.fireEvent(new mxEventObject('customFontsChanged', 'customFonts',
          this.customFonts));
      ...
      for (var i = 0;
          ...
          i++) {
        this.ui.editor.graph.addExtFont(this.previousExtFonts[i].name,
    ...
    this.put('fontFamily', new Menu(mxUtils.bind(this, function (menu, parent) {
                ...
                null, parent, function () {
                  ...
                  editorUi.fireEvent(new mxEventObject('styleChanged',
                      ...
                      'values', (urlParams['ext-fonts'] != '1') ?
                          [fontName,
                            (fontUrl != null) ? encodeURIComponent(fontUrl)
                                : null, null] : [fontName],
                      'cells', [graph.cellEditor.getEditingCell()]));
                }, function () {
                  graph.updateLabelElements(graph.getSelectionCells(),
                      function (elt) {
              ...
              mxEvent.addListener(img,
                  ...
                  mxUtils.bind(this, function (evt) {
                    if (urlParams['ext-fonts'] != '1') {
                      ...
                      var change = new ChangeExtFonts(this.editorUi, extFonts,
                          customFonts);
      ...
      if (urlParams['ext-fonts'] != '1') {
        ...
        menu.addItem(mxResources.get('reset'), null,
            mxUtils.bind(this, function () {
        ...
        if (extFonts != null && extFonts.length > 0) {
          ...
          if (changed) {
            this.editorUi.fireEvent(
                new mxEventObject('customFontsChanged', 'customFonts',
                    this.customFonts));
          }
        }
      ...
      menu.addItem(mxResources.get('custom') + '...', null,
          mxUtils.bind(this, function () {
            ...
            if (urlParams['ext-fonts'] != '1' && graph.isEditing()) {
              ...
              if (state != null) {
                curFontName = state.style[mxConstants.STYLE_FONTFAMILY]
                    || curFontName;
                ...
                if (urlParams['ext-fonts'] != '1') {
                  ...
                  if (curType == 'w') {
                    ...
                    curUrl = webFont != null ? webFont.url : mxResources.get(
                        'urlNotFound', null, 'URL not found');
                  }
                }
              }
            }

            if (curUrl != null && curUrl.substring(0, PROXY_URL.length)
                == PROXY_URL) {
            ...
            var dlg = new FontDialog(this.editorUi, curFontName, curUrl,
                curType, mxUtils.bind(this, function (fontName, fontUrl, type) {
                        ...
                        if (urlParams['ext-fonts'] != '1') {
                          ...
                          if (type != 's') {
                            ...
                            if (fontUrl.indexOf('http://') == 0) {
                              fontUrl = PROXY_URL + '?url='
                                  + encodeURIComponent(fontUrl);
                            }

                            this.editorUi.editor.graph.addExtFont(fontName,
                                fontUrl);
                          }
                        }
            ...
            this.editorUi.showDialog(dlg.container, 380,
                130 + (Editor.enableWebFonts ? 70 : 0)
                + (urlParams['isGoogleFontsEnabled'] != '0' ? 50 : 0), true,
                true);
            dlg.init();
          }), parent, null, true);
    })));
